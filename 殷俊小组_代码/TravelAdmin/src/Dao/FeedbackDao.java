...
public class FeedbackDao
{
    public List<Feedback> list() {
        ...
        try {
            ...
            while (resultSet.next()) {//循环获取每个查询结果
                ...
                //将获取到的字段值封装到一个 Feedback 对象中，再将这个对象添加到 List 集合中
                list.add(new Feedback(content,id,reply,username));
            }
    ...
    public Integer save(Feedback feedback) throws ParseException {
        // 获取数据库连接
        Connection connection = JDBCUtil.getConnection();
        // SQL 语句，？是占位符
    ...
    public Integer update(Feedback feedback) {
        ...
        try {
            statement = connection.prepareStatement(sql);
            // 为占位符赋值，第一个参数是第几个占位符，第二个参数是要赋的值
            statement.setString(1, feedback.getReply());
    ...
    public Integer delete(String content) {
        // 获取数据库连接
        Connection connection = JDBCUtil.getConnection();
        // SQL 语句
        String sql = "DELETE FROM messages WHERE content = '" + content + "'";
        PreparedStatement statement = null;
        Integer result = null;
